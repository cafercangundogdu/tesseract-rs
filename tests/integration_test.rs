...
fn test_multiple_languages_with_lstm() {
    ...
    let img = image::open("tests/test_images/multilang_sample.png")
        .expect("Failed to open image");
    let preprocessed = preprocess_image(&img);
    let (width, height) = preprocessed.dimensions();

    api.set_image(preprocessed.as_raw(), width as i32, height as i32, 1, width as i32);
    ...
    assert!(
        ...
        "Text does not contain expected words. Found: {}", text
    );

    let confidences = api.get_word_confidences();
    println!("Word confidences: {:?}", confidences);
    assert!(!confidences.is_empty(), "No word confidences returned");
    assert!(confidences.iter().any(|&c| c > 80), "No high confidence words found");
}
...
fn test_ocr_on_real_image() {
    ...
    assert!(confidences.iter().all(|&c| c > 0));
}
...
fn test_multiple_languages() {
    let tessdata_dir = std::env::var("TESSDATA_PREFIX").expect("TESSDATA_PREFIX not set");
    let api = TesseractAPI::new();
    api.init(&tessdata_dir, "eng+tur").expect("Failed to initialize Tesseract with multiple languages");
    api.set_variable("tessedit_pageseg_mode", "1").expect("Failed to set PSM");
    //api.set_variable("tessedit_char_blacklist", "!?@#$%&*()_+-=[]{}").expect("Failed to set char blacklist");
    api.set_variable("tessedit_enable_dict_correction", "1").expect("Failed to enable dictionary correction");
...
fn test_digit_recognition() {
    ...
    let api = TesseractAPI::new();
    api.init(&tessdata_dir, "eng").expect("Failed to initialize Tesseract");
    api.set_variable("tessedit_char_whitelist", "0123456789").expect("Failed to set whitelist");
